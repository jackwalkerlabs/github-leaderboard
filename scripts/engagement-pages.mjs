...
export function profileCard(profile, data) {
  const scope = `All-time stars · Starboard curated sample (${data.sampleSize} developers)`;
  return { key: `profile-${profile.id}`, title: profile.login, primary: `#${profile.rank}`, secondary: `${num(profile.stars)} stars · ${num(profile.projects)} projects`, scope,
    ...
    text: `GitHub builder ${profile.login} is #${profile.rank} by all-time stars in Starboard's curated sample.\n${num(profile.stars)} stars across ${num(profile.projects)} projects.\nSnapshot: ${day(profile.observedAt)}.` };
}
export function growthCard(entry, board, league = 'all') {
  ...
  return { key: `${period}-${league}-${entry.id}`, title: entry.login, primary: `#${entry.rank}`, secondary: `${delta(entry.gain)} net stars · ${entry.matched} matched projects`, scope,
    ...
    text: `GitHub builder ${entry.login} is #${entry.rank} in Starboard's ${period} ${league === 'rising' ? 'Rising builders' : 'All builders'} ranking.\n${delta(entry.gain)} net stars across ${entry.matched} matched projects.\n${board.baselineDate}–${board.endDate} · Curated sample.` };
}
export function shareCards(data) {
  return [
    ...data.profiles.map(profile => profileCard(profile, data)),
...
export function socialPayload(data, profiles = data.profiles) {
  ...
  return { origin: data.origin, profiles, week: { status: data.week.status, endDate: data.week.endDate }, cards: shareCards(data).filter(card => ids.has(Number(card.key.split('-').at(-1)))) };
}
export function profileCompetition(profile, data) {
  ...
  return `<section class="competition-profile" aria-label="Rank and following"><div><span class="eyebrow">YOUR PLACE ON STARBOARD</span><p><strong>#${profile.rank}</strong> by all-time stars <span>in our ${data.sampleSize}-developer sample</span></p>${week ? `<a href="/trending/week/">#${week.rank} this week · ${delta(week.gain)} net stars →</a>` : `<a href="/trending/week/">${pending} →</a>`}</div><div class="competition-profile-actions">${followButton(profile.id)}${shareButton(`profile-${profile.id}`)}<small>Following is saved in this browser.</small></div></section>`;
}
export function competitionTeaser(data) {
  const ready = data.week.status === 'ready';
  return `<section class="competition-teaser" aria-labelledby="competition-teaser-title"><div><span class="eyebrow">A REASON TO CHECK BACK</span><h2 id="competition-teaser-title">Follow the weekly climb.</h2><p>${ready ? `See who gained the most stars from ${data.week.baselineDate} to ${data.week.endDate}.` : 'Pick your builders. Follow their progress as our first week takes shape.'}</p></div><div class="profile-actions"><a class="profile-button claim-primary" href="/trending/week/">Explore the competition →</a><a class="profile-button" href="/following/">Your following</a></div></section>`;
}
...
function leaderboard(entries, board) {
  ...
  return `<ol class="climb-list">${entries.map(entry => `<li><span class="climb-rank">#${entry.rank}</span><div class="climb-person"><a href="${path(entry.login)}">${esc(entry.name)}</a><small>GitHub: ${esc(entry.login)} · ${entry.matched} matched projects${entry.added || entry.removed ? ` · ${entry.added} added / ${entry.removed} removed excluded` : ''}</small></div><div class="climb-gain"><strong>${delta(entry.gain)}</strong><small>net stars</small>${entry.toNextRank !== null ? `<span>${num(entry.toNextRank)} to pass the next rank</span>` : '<span>Top score in this league</span>'}</div><div class="climb-actions">${followButton(entry.id)}${shareButton(`${board.days === 7 ? 'weekly' : 'monthly'}-${board.league}-${entry.id}`)}</div></li>`).join('')}</ol>`;
}
...
export function storiesPage(data) {
  ...
  const spotlight = cards.filter(card => card.key.startsWith('profile-')).slice(0, 3);
  ...
  const choices = [...movement, ...spotlight];
  return { path: '/share/', title: 'Leaderboard stories to share — Starboard', description: 'Share developer spotlights and evidence-backed leaderboard updates. Preview post drafts and download Starboard rank cards.', crumbs: [['Leaderboard stories']], index: false,
    body: `<div class="page-intro"><span class="eyebrow">GIVE PUBLIC WORK SOME CREDIT</span><h1>A story worth <span>sharing.</span></h1><p>Drafts for your feed: developer spotlights now, fresh climbs when history is ready. Preview a post, download its card, and make it your own.</p></div><div class="story-grid">${choices.map(card => `<article class="story-card"><span class="eyebrow">${card.key.startsWith('profile-') ? 'DEVELOPER SPOTLIGHT' : 'LEADERBOARD UPDATE'}</span><h2>${esc(card.title)}</h2><p class="story-draft">${esc(card.text)}</p><div class="profile-actions">${shareButton(card.key)}<a class="text-button" href="${card.path}">View source →</a></div></article>`).join('')}</div><p class="page-note">GitHub usernames are shown as names, not assumed X handles. Every card includes its date and ranking scope. Posts open as drafts for you to review.</p>` };
}
