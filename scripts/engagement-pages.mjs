...
export function profileCompetition(profile, data) {
  ...
  return `<section class="competition-profile" aria-label="Rank and following"><div><span class="eyebrow">YOUR PLACE ON REPO LEAGUE</span><p><strong>#${profile.rank}</strong> by all-time stars <span>in our ${data.sampleSize}-developer sample</span></p>${week ? `<a href="/trending/week/">#${week.rank} this week · ${delta(week.gain)} net stars →</a>` : `<a href="/trending/week/">${pending} →</a>`}</div><div class="competition-profile-actions">${followButton(profile.id)}${shareButton(`profile-${profile.id}`)}<small>Following is saved in this browser.</small></div></section>`;
}
