...
export async function generateSite({ snapshot, template, outDir, origin = null, history = [], ledger = [], asOf = snapshot.fetched_at }) {
  ...
  const card = repo => `<article class="project"><div class="project-owner"><a href="${developerPath(repo.owner.login)}"><img src="${escape(repo.owner.avatar_url)}" alt="" width="21" height="21" loading="lazy">${escape(repo.owner.login)}</a>${repo.archived ? '<span class="archived">Archived</span>' : ''}</div><div class="project-top"><a href="${escape(repo.html_url)}" target="_blank" rel="noopener noreferrer">${escape(repo.name)} ↗</a><span>★ ${number(repo.stargazers_count)}</span></div><p>${escape(repo.description || 'No description provided on GitHub.')}</p><div class="repo-meta">${languageLink(repo.language)}<span>${escape(repo.license.spdx_id)}</span><span>${number(repo.forks_count)} forks</span></div><div class="push-date">Last push ${date(repo.pushed_at)}</div></article>`;
  ...
  const compactProjects = repos => `<ul class="compact-projects">${repos.map(repo => `<li><a href="${escape(repo.html_url)}" target="_blank" rel="noopener noreferrer">${escape(repo.full_name)} ↗</a><span>★ ${number(repo.stargazers_count)}</span></li>`).join('')}</ul>`;
