...
export async function listingRequest(request, env, getUser, githubFetch = fetch) {
  ...
  if (url.origin !== env.APP_ORIGIN || request.headers.get('Origin') !== env.APP_ORIGIN) throw new HttpError(403, 'Please use Starboard directly.');
  ...
  const response = await githubFetch(`https://api.github.com/user/${id}`, { headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'Starboard', 'X-GitHub-Api-Version': '2026-03-10' }, signal: AbortSignal.timeout(10000) });
  if (!response.ok) throw new HttpError(503, 'GitHub could not verify your public account. Please try again later.');
  const profile = await response.json();
  if (String(profile.id) !== id || profile.type !== 'User' || !/^[a-z\d][a-z\d-]{0,38}$/i.test(profile.login)) throw new HttpError(400, 'Starboard currently lists personal GitHub accounts.');
